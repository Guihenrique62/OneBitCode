...
const Spaceships = [];
do {
    input = prompt(`- Selecione a opção:
   ...
   `);
    switch (input) {
        case '1':
            ...
            const crewLimit = parseFloat(prompt('Digite a quantidade maxima de tripulantes suportada: '));
            const space = saveSpaceship(name, pilot, crewLimit);
            Spaceships.push(space);
            break;
        case '2':
            const nameOfSpaceship = prompt(`Escolha a nave que deseja inserir o tripulante: `);
            let spaceship;
            Spaceships.forEach((obj) => { spaceship = (obj.name === nameOfSpaceship) ? obj : spaceship; });
            const crew = prompt('Digite o nome do Tripulante: ');
            addCrew(crew, spaceship);
            break;
        case '3':
            const nameOfSpaceshiptoSend = prompt(`Escolha a nave que deseja enviar para uma missão: `);
            let spaceshiptoSend;
